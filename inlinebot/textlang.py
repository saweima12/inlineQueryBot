...
HELP_MSG="""
*使用方式*
- 傳送一張貼圖或是 GIF 或者 Audio 可以取得當前登錄的關鍵字及狀態。
- Reply 一張貼圖或是 GIF 或者 Audio 並輸入 /ak 可增加該資源的關鍵字。
- Reply 一張貼圖或是 GIF 或者 Audio 並輸入 /sk 可置重新設置該資源的關鍵字（覆蓋原有設置）

*注意事項*
- 關鍵字必須用半形逗號,隔開中間不可有其他的空白。
- 關鍵字添加後會由資料庫處理索引，需要幾分鐘的時間。

當前共支援 Audio | Sticker | GIF
"""
